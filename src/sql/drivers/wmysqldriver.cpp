...
the list \em first and \em then execute a query and fetch the result set using
getResult().
...
WSqlTable WMysqlDriver::tableMetaData ( const std::string &tableName )
{
	...
	while ( !record.isEmpty() )
	{
		...
		*/
		column.setColumnName ( record.field ( 0 ).data<std::string>() );
		column_names.push_back ( column.columnName() );
		initColumnType ( column, record.field ( 1 ).data<std::string>() );
		bool nullable = ( record.field ( 2 ).data<std::string>().compare ( "NO" ) == 0 );
		bool primarykey = ( record.field ( 3 ).data<std::string>().compare ( "PRI" ) == 0 );
		column.setDefaultValue ( record.field ( 4 ).data<std::string>() );
		bool isauto = ( record.field ( 5 ).data<std::string>().find ( "auto_increment" ) != std::string::npos );
		column.setCanBeNull ( nullable );
		column.setIsPrimaryKey ( primarykey );
		column.setIsAutoIncremented ( isauto );
		tblToReturn.append ( column );
